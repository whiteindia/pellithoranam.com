...
//	if($_POST) {
  ...
  if($intrst_count['status'] == "success") {
	...
	public function membershipinfo()
	{
		...
		$my_matr_id = $this->session->userdata('logged_in');
		$membership = $this->Profile_model->get_membership_details($my_matr_id->matrimony_id);
		...
		//$query1 = $this->db->where('mobileview_to',$profile[0]->matrimony_id); 
		 ...
		 $result['mobileview_count_total']=$membership->total_mobileview;
		 $result['mobileview_count_used']=$query1->num_rows();
